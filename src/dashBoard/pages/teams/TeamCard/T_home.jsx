...
const T_home = () => {
  ...
  const fetchPost = async () => {
    try {
      ...
      //    url: ,
      // })
      getData(`/technologies`).then((res) => {
        setFetch(res?.AllTechnologies);
        setLoader(false);
      });
    } catch (err) {
      console.log(err);
  ...
  const fetchEmployees = async () => {
    try {
      await getData(`/${route}`).then((res) => {
        setEmployees(res?.employees);
        setSpinner(false);
        if (route === "employees") {
          all.current = res.employees.length;
        }
      });
    } catch (err) {
      console.log(err);
  ...
  const fetchCards = async () => {
    try {
      ...
      //    url: ,
      // })
      getData(`/subdepartments`).then((res) => {
        setCards(res?.Subdepartments);
        setLoader(false);
      });
    } catch (err) {
      console.log(err);
      ...
      <div className="dashboard_allfields" style={{ gap: "2rem" }}>
        <form style={{ padding: "1rem 1.5rem" }}>
          <div className="team_cards" ref={scrollRef}>
            ...
            >
              <div className="team_cards-card_img">
                {" "}
                <img src={card_icon} alt=" card icon" />{" "}
              </div>
              <div className="team_cards-card_content">
                {" "}
                <h4>All Employee</h4> <span>{all.current}</span>{" "}
              </div>
            ...
            {cards.map((card, index) => {
              return (
                ...
                >
                  ...
                  <div className="team_cards-card_content">
                    {" "}
                    <h4>{card.title}</h4> <span>{card.employee_count}</span>{" "}
                  </div>
              ...
              {!!employees.length ? (
                employees?.map((person) => {
                  return (
                    ...
                    >
                      ...
                      <p>
                        {person.technologies && person?.technologies[0]?.name}
                      </p>
                    </div>
                  );
                })
                ...
                <div className="flex justify-center" style={{ width: "100%" }}>
                  ...
                  <h5 style={{ color: "var(--blue)" }}>
                    No Employees in this Specialization
                  </h5>
                </div>
              )}
            </div>
